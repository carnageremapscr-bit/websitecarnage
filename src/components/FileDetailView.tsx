      ...
      {messages.map((msg, i) => (
        <div key={i} className={`chat-message mb-2 p-2 rounded ${msg.sender === "You" ? "bg-yellow-700 self-end" : "bg-gray-800 self-start"}`}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400" title={typeof msg.time === 'string' ? msg.time : (typeof msg.time === 'number' ? String(msg.time) : undefined)}>{msg.time ? new Date(typeof msg.time === 'number' || typeof msg.time === 'string' ? msg.time : Date.now()).toLocaleString() : ''}</span>
          </div>
